...
function transformEntry(entry: any): BlogPost {
  const fields = entry.fields as BlogPostFields;
  
  // Transform rich text content to HTML string using Contentful's renderer
  const content = fields.content
    ? documentToHtmlString(fields.content as any)
    : "";
...
export async function getAllBlogPosts(): Promise<BlogPost[]> {
  try {
    ...
    try {
      ...
      // If ordering fails (field might not exist), try without ordering
      ...
      if (
        ...
        orderError?.details?.errors?.[0]?.name === "invalidOrder"
      ) {
...
export async function getBlogPostBySlug(slug: string): Promise<BlogPost | null> {
  try {
    ...
    try {
      ...
      // If slug field doesn't exist, try getting all entries and finding by ID
      if (slugError?.details?.errors?.[0]?.path?.includes("slug")) {
...
export async function getAllBlogPostSlugs(): Promise<string[]> {
  try {
    ...
    try {
      ...
      // If slug field doesn't exist, fallback to entry IDs
      if (slugError?.details?.errors?.[0]?.path?.includes("slug")) {
    ...
    if (error?.details?.errors?.[0]?.name === "unknownContentType") {
      ...
      console.error("Error fetching blog post slugs:", error?.message || error);
    }
    return [];
  }
}
